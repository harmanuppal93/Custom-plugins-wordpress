		...
		if (is_user_logged_in() && current_user_can('edit_booked_appointments')):
			...
			if (!isset($atts['remove_wrapper'])): echo '<div id="booked-profile-page" class="booked-shortcode">'; endif;
				...
				echo '<div class="booked-fea-appt-list">';
					...
					if ($pending):
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s Pending Appointment','%s Pending Appointments',$total_appts,'booked-frontend-agents'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="booked-no-margin">'.__('No pending appointments.','booked-frontend-agents').'</p>';
						endif;
					elseif ($historic):
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s Past Appointment','%s Past Appointments',$total_appts,'booked-frontend-agents'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="booked-no-margin">'.__('No past appointments.','booked-frontend-agents').'</p>';
						endif;	
					else:
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s Upcoming Appointment','%s Upcoming Appointments',$total_appts,'booked-frontend-agents'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="booked-no-margin">'.__('No upcoming appointments.','booked-frontend-agents').'</p>';
						endif;
					endif;
					...
					foreach($appointments_array as $appt):
						...
						if ($date_display == $today){
							$date_display = __('Today','booked-frontend-agents');
							$day_name = '';
						} else {
						...
						if ($timeslots[0] == '0000' && $timeslots[1] == '2400'):
							...
							$timeslotText = (!get_option('booked_hide_end_times') ? __('from','booked-frontend-agents').' ' : __('at','booked-frontend-agents').' ') . $time_start . (!get_option('booked_hide_end_times') ? ' ' . __('to','booked-frontend-agents').' '.$time_end : '');
							$google_date_startend_end = $google_date_startend;
						endif;

						$status = ($appt['status'] !== 'publish' && $appt['status'] !== 'future' ? __('pending','booked-frontend-agents') : __('approved','booked-frontend-agents'));
						...
						echo '<div class="appt-block bookedClearFix" data-appt-id="'.$appt['post_id'].'">';
							...
							if (!isset($appt['guest_name'])):
								...
								if (isset($user_info->ID)):
									...
									$user_display = __('(this user no longer exists)','booked-frontend-agents');
								endif;
							else :
			...
			return '<p>'.__('Please log in to view your upcoming appointments.','booked-frontend-agents').'</p>';
